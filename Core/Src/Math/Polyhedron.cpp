...
namespace Sapphire
{
	...
	void Polyhedron::Clip(const Plane& plane)
	{
		...
		if (clippedVertices_.size() > 3)
		{
			...
			outFace_.push_back(*clippedVertices_.begin());
	...
	void Polyhedron::Transform(const Matrix3x3& transform)
	{
		for (unsigned i = 0; i < faces_.size(); ++i)
		{
			std::vector<Vector3>& face = faces_[i];
	...
	Polyhedron Polyhedron::Transformed(const Matrix3x3& transform) const
	{
		Polyhedron ret;
