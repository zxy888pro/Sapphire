...
namespace Sapphire
{
	...
	Matrix3x3 Deserializer::ReadMatrix3x3()
	{
		float data[9];
		Read(data, sizeof data);
	...
	Matrix4x4 Deserializer::ReadMatrix4x4()
	{
		float data[16];
		Read(data, sizeof data);
	...
	std::vector<byte> Deserializer::ReadBuffer()
	{
		std::vector<byte> ret(ReadVLE());
		if (ret.size())
			Read(&ret[0], ret.size());
		return ret;
	}
	...
	Variant Deserializer::ReadVariant(VariantType type)
	{
		...
		case VAR_MATRIX3X3:
			return Variant(ReadMatrix3x3());

		case VAR_MATRIX3X4:
			return Variant(ReadMatrix3x4());

		case VAR_MATRIX4X4:
			return Variant(ReadMatrix4x4());
