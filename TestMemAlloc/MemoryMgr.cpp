...
*/
void *AllocateMemory(const char *file, int line, size_t size, AllocType type, void *address)
{
	...
	// this error.
	m_assert(type != AllocType::AT_Unknown);

	if (type == AllocType::AT_Realloc) {
		...
		m_assert(memory->allocationType == AllocType::AT_Malloc ||
			memory->allocationType == AllocType::AT_Calloc ||
			memory->allocationType == AllocType::AT_Realloc);
	...
	if (type == AllocType::AT_Calloc) memory->InitializeMemory(0x00000000);
...
void deAllocateMemory(const char *file, int line, void *address, AllocType type)
{
	...
	m_assert(type == AT_Delete       && memory->allocationType == AT_New ||
		type == AT_DeleteArray && memory->allocationType == AT_NewArray ||
		type == AT_Free         && memory->allocationType == AT_Malloc ||
		type == AT_Free         && memory->allocationType == AT_Calloc ||
		type == AT_Free         && memory->allocationType == AT_Realloc);
